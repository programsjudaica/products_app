...
- If ALL provided photos are angled/perspective shots (three-quarter view, hero/lifestyle angle) and none is a plain straight-on orthographic front view, you MUST add a question asking for one: something like "Please add a straight-on front photo (no angle, plain background) for an accurate trace - all provided photos are angled, so this silhouette is only a rough approximation." Angled photos structurally cannot be traced into an accurate front elevation no matter how carefully you look - say so plainly instead of quietly guessing.
- If a part has protruding elements that stick out from the main body in ways a single closed silhouette can't capture well (e.g. side handles on a cup), split that protrusion into its own separate part too, the same way you would split a physically separate accessory piece.
